...
--
-- No foreign key to PREFIX_events, in either direction. Events name their keys through ordinary
-- dek: tags, and a constraint would either block pruning events or cascade keys away with them.

CREATE TABLE IF NOT EXISTS PREFIX_shredding_keys (
...
CREATE UNIQUE INDEX IF NOT EXISTS PREFIX_idx_shredding_keys_active
    ON PREFIX_shredding_keys (subject_type, subject_id, subject_category)
...
CREATE INDEX IF NOT EXISTS PREFIX_idx_shredding_keys_subject
    ON PREFIX_shredding_keys (subject_type, subject_id, subject_category);
