-- A database as an OLDER RELEASE of this library would have created it. Used by
-- PostgresSchemaUpgradeTest to prove that running the current version against an existing
-- eventstore upgrades it in place rather than requiring a new one. Derived from
-- ensure-schema.sql at ff465a9, then aged: the pre-alignment stream_purpose default, the
-- table-wide UNIQUE on idempotency_key, no combined stream+tags index, and a notify function
-- whose payload predates eventTx.
...
CREATE TABLE IF NOT EXISTS PREFIX_events (
	...
	-- (btree_gin and the combined stream+tags index did not exist yet)
	...
	-- (the per-stream partial unique index did not exist yet)
